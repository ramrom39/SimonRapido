...
function accionSimon() {
    if (juegoIniciado) {
        ...
        secuenciaSimon.push(colores[random]); //añade un nuevo color a la secuencia
            setTimeout(() => pulsaBoton(colores[random]),
                1 * 700); // por cada color en la secuencia, añade segundos equivalentes para que se vea su pulsación y no se superpongan (de lo contrario, visualmente los pulsaria a la vez, aunque haya un orden)
    }
    setTimeout(() => {
        turnoJugador = true;
